...
public class JpaConfig {
    ...
    public AuditorAware<String> auditorAware() {
        ...
        return () -> Optional.ofNullable(SecurityContextHolder.getContext())
                ...
                .filter(Authentication::isAuthenticated) /* isAuthenticated: filter 를 이용해서 인증이 됐는지 확인 하는 뜻. 로그인 한 상태인지 보라는거임.  */
                .map(Authentication::getPrincipal) /* Authentication 안에 Principal 이라는 인증정보 가져오라는 뜻 */
                .map(BoardPrincipal.class::cast) /* BoardPrincipal class 를 불러온다.  */
